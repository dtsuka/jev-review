...
const program = new Command()
  ...
  .option('-t, --threshold <number>', 'override all category thresholds')
...
const overrideThreshold = options.threshold === undefined ? undefined : Number(options.threshold);
const concurrency = Math.max(1, Number(options.concurrency));
const apiKey = process.env.TYPESAFE_API_KEY;

if (!apiKey) { console.error(pc.red('TYPESAFE_API_KEY is required.')); process.exit(1); }
if (overrideThreshold !== undefined && (!Number.isFinite(overrideThreshold) || overrideThreshold < 0 || overrideThreshold > 1)) {
  console.error(pc.red('--threshold must be between 0 and 1.')); process.exit(1);
}

const checks = parseChecks(options.checks);
const thresholds = { ...DEFAULT_THRESHOLDS };
if (overrideThreshold !== undefined) for (const check of CHECKS) thresholds[check] = overrideThreshold;
...
console.log(pc.dim(`Checks: ${checks.join(', ')}`));
console.log(pc.dim(`Thresholds: ${checks.map((c) => `${c}=${thresholds[c]}`).join(', ')}`));
...
const results = await mapLimit(files, concurrency, async (file): Promise<FileResult> => {
  ...
  try {
    ...
    const hits = Object.entries(scores).filter(([check, score]) => (score ?? 0) >= thresholds[check as CheckName]);
    if (hits.length) console.log(`${pc.yellow('⚠')} ${relative}  ${hits.map(([k,v]) => `${k} ${Math.round((v ?? 0)*100)}%`).join('  ')}`);
    else console.log(`${pc.green('✓')} ${pc.dim(relative)}`);
...
const report: ReviewReport = { version: 2, generatedAt: new Date().toISOString(), root: directory, thresholds, checks, files: results };
const outputDir = path.join(directory, '.jev-review');
await mkdir(outputDir, { recursive: true });
await writeFile(path.join(outputDir, 'report.json'), `${JSON.stringify(report, null, 2)}\n`);
const attention = results.filter((r) => Object.entries(r.scores).some(([check, score]) => (score ?? 0) >= thresholds[check as CheckName]));
...
async function mapLimit<T,R>(items:T[],limit:number,fn:(item:T)=>Promise<R>):Promise<R[]>{const results=new Array<R>(items.length);let next=0;async function worker(){while(true){const index=next++;if(index>=items.length)return;results[index]=await fn(items[index]);}}await Promise.all(Array.from({length:Math.min(limit,items.length)},worker));return results;}
